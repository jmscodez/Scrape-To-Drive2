...
reddit = praw.Reddit(
    ...
    user_agent="script:golfbot:v1.0 (by /u/YourUsername)"
)

if __name__ == "__main__":
    drive_service = authenticate_drive()
    folder_id = get_or_create_folder(drive_service, "golf")
    processed = 0
    target = 3  # how many videos per run

    for post in reddit.subreddit("golf").top(time_filter="day", limit=50):
        if processed >= target:
            break
